...
void solve() {
  ...
  for(P p:s){
    for(P q:s){
      ...
      bool f=false;
      P w,x,y,z; //頂点候補
      w=make_pair(c+b-d,d+a-c);
      x=make_pair(a+b-d,b+a-c);
      y=make_pair(c-b+d,d-c+a);
      z=make_pair(a-b+d,b-c+a);
      if(s.count(w)&&s.count(x)) f=true;
      if(s.count(y)&&s.count(z)) f=true;
      if(f) chmax(ans,POW(a-c,2)+POW(b-d,2));
    }
  }
  out(ans);
}
